...
>
> ### 2026-09-21 追記：ログインメールは「コードのみ」で送る（決定ログ §20-1）
>
> Supabase Auth の **Magic link or OTP テンプレートから `{{ .ConfirmationURL }}` を削除し、
> `{{ .Token }}`（6桁コード）だけを本文に置く**（オーナー決定）。
>
> アプリのログインは `verifyOtp({ type: "email", token })` の**リダイレクトを伴わない方式**であり、
> `emailRedirectTo`・`/auth/callback`・`exchangeCodeForSession` のいずれも実装に存在しない。
> つまり**サインインリンクは使われないまま本文に載っていた**。外すことで
> ①その環境の Site URL / Redirect URLs がログイン経路に影響しなくなり（設定ミスという失敗様式が消える）、
> ②メール転送・誤送信・受信箱の漏洩による乗っ取り経路が1本減る。
>
> ⚠️ **適用当初はログインのテンプレートのみで、招待（経路B ／ `inviteUserByEmail`）は
> Invite user という別テンプレートのリンク方式のまま残っていた**（アプリ側に戻り先の受け口が無い問題は
> `QUESTIONS.md`「[2026-09-21] 招待メール（経路B）のリンクを受ける導線がアプリ側に存在しない」で
> 一時未決だった）。→ **決定ログ §22-1（v13 §9 #67・2026-09-22）で招待も「コード方式」へ寄せ、
> `inviteUserByEmail()` 自体を廃止して解消済み**。招待は台帳登録＋ログイン画面案内メールの2段になり、
> リンクは全経路から消えている。
>
> ⚠️ **テンプレート編集にはカスタム SMTP の設定が先行して要る**（Supabase の制約。2026-09-21 実機確認）。
>
> **アプリの公開ドメインは Vercel 既定（`*.vercel.app`）**（決定ログ §20-2）。
> 本項の `fuyugai.jp` は**メールの送信ドメイン**であって、アプリのURLではない（両者は独立）。
