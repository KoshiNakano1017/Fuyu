...
| **21. 買い物リスト ＆ 買い出しクエスト化（v13 §5.12、2026-09-22 新設）** | **`shopping_list_items`（未起票）** | ⏸ **DDL 未作成。**要件は v13 §5.12 で確定しているが、**Phase の置き場所が v13 §9 #65 のオーナー判断待ち**のため本書ではまだ設計していない。設計時の要点は3つ。①**クエストとは多対1**（複数品目 → 買い出しクエスト1件。`quest_id` を品目側に持ち、`quests.source_type = 'shopping_list'`）②**立替実費は円**（`advance_amount_jpy`）で、報酬の Uii と**同一カラム・同一合計に混ぜない**③**`orders` の未会計額へマイナス計上しない**（請求の器と債務の器を分ける／v13 §5.12.4） |
