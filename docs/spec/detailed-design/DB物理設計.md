...
> [!important] ★ 2026-09-26 追記：占有は「**その夜に実際に使う形態**」で数える（WBS `3-10` ／ v13 §5.6.9）
> 滞在の途中で宿泊形態が変わりうる（§5.6.9）ため、`check_ins.room_type`（＝**現在の**形態）で
> 滞在の全泊を数えると、**「明日からコテージへ移る」変更を入れた瞬間に今夜のキャンプサイトが空き枠として返る**。
> 同じ枠へ別の予約を通してしまい、ダブルブッキングになる。
>
> `0041_check_in_changes.sql` がビューを差し替え、`public.check_in_state_on(checkin_id, date)` が返す
...
### 3-15. 滞在の変更履歴（`check_in_changes`）（v13 §5.6.9 ／ WBS `3-10`。2026-09-26 新設）

顧客管理画面から**滞在中の宿泊形態・部屋割当・退去日・人数**を変更できるようにするための表である
（決定ログ §25-2〜§25-5 ／ 実体は `supabase/migrations/0041_check_in_changes.sql`）。

> [!important] `check_ins` の上書きだけでは要件を満たせない
> §5.6.9 は宿泊費を「**その夜に実際に使った宿泊形態**の単価」で積むと定め、
> **滞在全体を新しい形態の単価で塗り替えない**ことを明示している。
> `check_ins.room_type` を上書きする実装では「3泊目から移った」事実が消え、
> 読み出した時点で3泊すべてが新しい形態になる。過去の夜の形態を復元できる場所が他に無いため、
> **履歴表が正本**であり、`check_ins` は「最後の変更の結果＝現在値」を持つ。

| 列 | 型 | 役割 |
| --- | --- | --- |
| `change_id` | uuid PK | |
| `checkin_id` | uuid NOT NULL → `check_ins` | 対象の滞在（`ON DELETE CASCADE`） |
| `effective_date` | date NOT NULL | ★ **この日の夜から**変更後の内容が効く。単価の切り替わる境界 |
| `room_type_before` / `_after` | text → `accommodation_types` | 変えていない項目は**両方 NULL** |
| `check_out_date_before` / `_after` | date | 同上 |
| `adults_before` / `_after`・`children_before` / `_after` | integer | 同上 |
| `room_id_after` | uuid → `rooms` | 移動先の部屋（未割当のままの変更もあるため NULL 可） |
| `reason` | text NOT NULL | ★ 理由必須（§5.6.9 ／ §5.6.4 の編集理由と同じ扱い） |
| `changed_by` | uuid NOT NULL → `members` | 誰が変えたか |
| `created_at` | timestamptz | |

- **追記専用**（UPDATE / DELETE のポリシーを置かない）。訂正は新しい変更行を理由つきで積む
  （`stay_ticket_transactions`〔`0016`〕・`member_notes`〔`0034`〕と同じ作法）
- RLS は **staff のみ**（SELECT / INSERT）。§5.6.9 の権限（管理者・コアメンバー）に合わせ、
  **本人向けの SELECT は置かない** — ここに残るのは運営の編集理由であり、本人に見せるのは §5.6.8 の宿泊履歴までである
- CHECK で守っていること: 理由が空白だけの行を作れない／変更が1つも無い行を作れない／
  「変更前 = 変更後」の行を作れない（＝「変えていない項目は NULL」の規約を DB 側で守らせる）

**`check_in_state_on(checkin_id, date)`**（`0041`）がその夜の形態・人数を返す。優先順は
① その夜までに効いた変更の最後の `*_after` ② 1件も無ければ最初の変更の `*_before`（＝滞在開始時の値）
③ 変更履歴が無ければ `check_ins` の現在値。
⚠️ **項目ごとに最後の非NULLを引く。** 1行が変わった項目だけを持つため、「最後の変更行を1行選んでその列を読む」
実装にすると、人数だけを直した変更のあとに形態が NULL で返る。残枠ビュー（§3-12）と宿泊費の算定が
**同じ答えを見る**ための唯一の出所である。

**`trg_rooms_room_type_immutable`**（`0041`）が `rooms.room_type` の UPDATE を `42501` で拒む。
v13 §5.6.9 の [!important]（形態は予約の属性であって部屋の属性ではない）を DB 側で担保するもので、
`service_role` も RLS を迂回するため関門はトリガーでなければならない（§6-6b と同じ理屈）。
部屋の用途を本当に変える場合は、その行を `status = '利用停止'` にして**新しい部屋を1行足す**
（`room_name_snapshot` に過去の割当が残る ／ v13 §5.6.8）。

> [!note] 宿泊費の差額には独自の精算経路を作らない（決定ログ §25-5）
> §5.6.9 の「差額の扱い」は §5.6.5／§5.6.6 の未処理差額へ載せると定めるが、
> **宿泊費を伝票として起こす経路が Phase 1 にまだ無い**（`orders` は注文のみ ／ `0019`）。
> 請求が無いまま `settlement_adjustments` へ行を積むと消し込めない差額が溜まるため、
> 本表の段階では**泊単位の内訳と合計を画面へ返す**ところまでを担う。
> 請求経路（`3-4`・`7-2` の延長）が入った時点で配線を足す。
...
| **21. 買い物リスト ＆ 買い出しクエスト化（v13 §5.12、2026-09-22 新設）** | **`shopping_list_items`（未起票）** | 🟢 **Phase 1 で実装（2026-09-22 オーナー確定 ／ v13 §9 #65）。DDL は実装時に起票する。**設計時の要点は3つ。①**クエストとは多対1**（複数品目 → 買い出しクエスト1件。`quest_id` を品目側に持ち、**`quests.origin_type = 'shopping_list'`**。⚠️ 実装済みの列名は `origin_type` であり `source_type` ではない。既存の `chk_quests_origin_type` を張り直して値を追加する）②**立替実費に相当するカラムを作らない**（`advance_*` は不要。アプリは買い出しの実費を扱わない／v13 §5.12.4・2026-09-22 確定）③**登録できるのは `admin` / `core_member` / `member` の3ロール**。**`guest` は SELECT のみ**を RLS で担保する |
