-- =============================================================================
-- 0102_rag_search_scope_and_pii.sql — 検索時スコープを DB 層へ固定し、PII 検出を連絡先へ広げる
--
--   ⚠️ **連番は 0100 番台である。** `rag` スキーマと `knowledge_chunks` を作るのは `0100` であり、
--      0041 として置くと `0100` より先に走って `schema "rag" does not exist` で落ちる
--      （CI で実際に踏んだ）。RAG 系は 0100 番台に並べる。
--
