-- ⚠️ 2026-09-19: 連番を 0007 → 0010 へ付け替えた。
-- Issue #53（quests）と #58（宿泊者名簿）が並列に実装され、どちらも 0007 を取ったため
-- `schema_migrations_pkey` の一意制約に衝突し、main の DB テストが全て落ちていた。
-- 依存を持たない本ファイル側を動かした（0008 は 0007_quests_schema に依存するため動かせない）。
-- 本番・dev とも未適用の段階であり、CLAUDE.md §4.5「既存マイグレーションを書き換えない」の
-- 目的（適用済みの履歴を壊さない）には反しない。
-- =============================================================================
