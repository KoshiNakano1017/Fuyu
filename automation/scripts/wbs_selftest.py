...
def main() -> int:
    ...
    # 2026-09-22: WBS 上「🔴 ブロック中」の作業パッケージ行が0件になった（2-5 が #58 決着で
    # 解除されたのが最後の1件だった）。is_blocked=True の実データ固定値は、次に本当にブロックされた
    # 行が現れたときに追加すること（is_dropped の2件は本項の趣旨と無関係なので維持する）。
    ...
    # ことで 5-7・12-4 も 🔴 → 🟢 へ変わり、続けて #58 決着で 2-5 も 🔴 → 🟢 へ変わった。
    # ⚠️ この結果、WBS 上「🔴 ブロック中」の作業パッケージ行は現在0件である。
    # want_blocked=True の実データ固定値は、次に本当にブロックされた行が現れたときに
    # 追加すること（パーサの True/False 判定そのものは §3b 冒頭のリテラル文字列検査が
    # 別途カバーしているため、実データ側が0件でも検査の抜けにはならない）。
    for package_id, want_blocked in [("2-2", False), ("2-4", False)]:
        package = packages.get(normalize_id(package_id))
        if package is None:
            failures.append(f"{package_id} が WBS に見つかりません（検査を更新してください）")
            continue
